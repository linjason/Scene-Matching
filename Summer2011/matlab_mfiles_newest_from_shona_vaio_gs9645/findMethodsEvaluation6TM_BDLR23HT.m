...
if ~exist('compsSideViewsAllBDLR23HT_Dnsz_Rsz','var')
    load('compsSideViewsAllBDLR23HT_Dnsz_Rsz');
end
...
taggedIndices=taggedIndices(5401:5409);
    ...
    if 1%isempty(compsSideViewsAllBDLR2Dnsized{candidate}) || (xySorted(1)<10 || xySorted(2)<10 || height<15 || height>50 || xySorted(1)>90 || xySorted(2)>90) || isempty(compsSideViewsAllBDLR2Dnsized{candidate}) || isempty(aaBDLR2{candidate})
        %if ~isempty(compsSideViewsAllBDLR2Dnsized{candidate}) && (xySorted(1)>=10&xySorted(2)>=10&& height>13&& height<75&&xySorted(1)<=90&&xySorted(2)<=90)&&~isempty(compsSideViewsAllBDLR2Dnsized{candidate}) && ~isempty(aaBDLR2{candidate})
            newTaggedIndices(i)=1;
             finalScores{i}=(evaluateTempMatchHS2(validTemplates',taggedIndices(i),compsSideViewsAllBDLR23HT_Dnsz_Rsz,compsHeightViewsAllBDLR23HT_Dnsz,0)/30);
             %end
