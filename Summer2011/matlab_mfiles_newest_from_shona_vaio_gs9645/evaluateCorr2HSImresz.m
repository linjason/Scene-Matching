function [scores]=evaluateCorr2HSImresz(search, candidate,compsSideViewsAllRsz,aaRsz,C)
...
%    load('../../dataStructureForStatistics/bedrooms_2_with_dist_nametags');
    ...
    i;
        if isempty(aaRsz{i}) || (xySorted(1)<10 || xySorted(2)<10 || height<15 || height>50 || xySorted(1)>90 || xySorted(2)>90) || isempty(compsSideViewsAllRsz{i})
        scores=0;
    else
        scores=zeros(0,0);
