% for each component in each scene, computes its distance to the walls on 4 sides and stores them in the
% B cell array (same thing as the C array, just change its name afterwards)
% First run readInValuesAndExtractMinMax to generate all other entries in B, then run this on the same mat files
