...
bedIndices=[1:length(comps3DVoxBDLR23HT_Dnsz_Rsz)]';
...
parfor i=1:size(taggedIndices,1)
    ...
    %scores=zeros(0,4);
    if 1%isempty(compsSideViewsAllBDLR2Dnsized{candidate}) || (xySorted(1)<10 || xySorted(2)<10 || height<15 || height>50 || xySorted(1)>90 || xySorted(2)>90) || isempty(compsSideViewsAllBDLR2Dnsized{candidate}) || isempty(aaBDLR2{candidate})
        %finalScores{i}=ones([1
        %length(validTemplates)]).*100000000;
    ...
    finalScores{i}=evaluateCorr2HSImresz2(validTemplates',taggedIndices(i),compsSideViewsAllBDLR23HT_Dnsz_Rsz5050,compsHeightViewsAllBDLR23HT_Dnsz_Rsz5050,compsHeightViewsAllBDLR23HT_Dnsz,0);
    %end
    else
        % finalScores{i}=evaluateCorr2HSImresz2(validTemplates',taggedIndices(i),compsSideViewsPorBDLR2Rsz,aaBDLR2Rsz,C);
    end
    toc
end
toc
