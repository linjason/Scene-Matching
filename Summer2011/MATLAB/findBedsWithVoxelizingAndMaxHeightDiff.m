...
% clear comps;
for i=1:size(C,2)
    if (~isempty(findstr('bed',lower(C{i}{7}))) && isempty(findstr('lateral',lower(C{i}{7}))) && isempty(findstr('head',lower(C{i}{7}))) && isempty(findstr('table',lower(C{i}{7}))) && isempty(findstr('frame',lower(C{i}{7}))) && isempty(findstr('room',lower(C{i}{7}))) && isempty(findstr('latte',lower(C{i}{7}))) && isempty(findstr('tv',lower(C{i}{7}))) && isempty(findstr('leg',lower(C{i}{7}))) && isempty(findstr('pole',lower(C{i}{7}))))
...
%             otherScore=-1.0*abs(1-(max(oneSize(1),otherSize(1))/min(oneSize(1),otherSize(1))));
    ...
    if abs(heightValueForTemplate-heightValueForImage)<100000
        ...
        C{i}
        i;
        newBeds6{bedIndex}{1}=compsHeightViews;
        newBeds6{bedIndex}{2}=scores;
        newBeds6{bedIndex}{3}=i;
    end
end
toc