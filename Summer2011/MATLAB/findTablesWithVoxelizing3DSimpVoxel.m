function [d, indices, newIndices]=findTablesWithVoxelizing3DSimpVoxel()
...
for i=1:size(C,2)
    ...
    if (~isempty(findstr('table',lower(C{i}{7}))) )&& isempty(findstr('lamp',lower(C{i}{7}))) && isempty(findstr('bed',lower(C{i}{7}))) && isempty(findstr('leg',lower(C{i}{7}))) && isempty(findstr('dresser',lower(C{i}{7}))) && isempty(findstr('convertable',lower(C{i}{7})))
...
parfor i=1:size(C,2)
    ...
    if (xySorted(1)<10 || xySorted(2)<10 || height<15 || height>45 || xySorted(1)>60 || xySorted(2)>60)
...
if size(find(min(scores')<3700),2)>0&&topScore>0.7
