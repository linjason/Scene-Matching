function [d, indices, newIndices]=findBedsWithVoxelizing3DSimpVoxel()
...
for i=1:size(C,2)
 ...
 if ((~isempty(findstr('bed',lower(C{i}{7})))) || ~isempty(findstr('couch',lower(C{i}{7}))))&& isempty(findstr('lateral',lower(C{i}{7}))) && isempty(findstr('head',lower(C{i}{7}))) && isempty(findstr('table',lower(C{i}{7}))) && isempty(findstr('frame',lower(C{i}{7}))) && isempty(findstr('room',lower(C{i}{7}))) && isempty(findstr('latte',lower(C{i}{7}))) && isempty(findstr('tv',lower(C{i}{7}))) && isempty(findstr('leg',lower(C{i}{7}))) && isempty(findstr('pole',lower(C{i}{7}))) && isempty(findstr('locker',lower(C{i}{7})))
        sizesForComp = C{i}{4} - C{i}{3};
        if sizesForComp(3)<15 || sizesForComp(3)>50 || sizesForComp(3)<15 || sizesForComp(1)<20 || sizesForComp(2)<20
            continue;
        end
        j=j+1;
        C{i};
...
parfor i=1:size(C,2)
    ...
    if (xySorted(1)<20 || xySorted(2)<20 || height<29 || height>70 || xySorted(1)>110 || xySorted(2)>110)
    ...
    for j=1:size(indexIntoIndices,2)
        ...
        scores(j,1)=scores1/exp((oneScore+otherScore)*1.4);
        scores(j,2)=scores2/exp((oneScore2+otherScore2)*1.4);
        scores(j,3)=scores3/exp((oneScore+otherScore)*1.4);
        scores(j,4)=scores4/exp((oneScore2+otherScore2)*1.4);
    end
    

    %if size(find(max(scores')>0.5),2)>0&&topScore>0.65
    if size(find(min(scores')<12000),2)>0&&topScore>0.6
