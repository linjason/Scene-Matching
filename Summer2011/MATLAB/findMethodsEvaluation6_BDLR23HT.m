tic
disp('wrg')
load('compsHeightViewsAllBDLR23HT_Dnsz');
%load('compsSideViewsAll');
if ~exist('comps3DVoxBDLR23HT_Dnsz_Rsz','var')
    load('comps3DVoxBDLR23HT_Dnsz_Rsz')
end
...
for sliceIndex=1:ceil(length(comps3DVoxBDLR23HT_Dnsz_Rsz)/200)
    %[1:55 57:ceil(length(C)/200)]
    ...
    for i=1:length(comps3DVoxBDLR23HT_Dnsz_Rsz)
    ...
    bedIndices=[1+200*(sliceIndex-1):min(200*sliceIndex,length(comps3DVoxBDLR23HT_Dnsz_Rsz))]';
    1+200*(sliceIndex-1)
    min(200*sliceIndex,length(comps3DVoxBDLR23HT_Dnsz_Rsz))
    ...
    for j=1:length(bedIndices)
        if ~isempty(comps3DVoxBDLR23HT_Dnsz_Rsz{bedIndices(j)})
    ...
    for k=1:4
        ...
        for j=1:length(validTemplates)
            if isempty(comps3DVoxBDLR23HT_Dnsz_Rsz{validTemplates(j)})
                disp('oh oh 10')
                continue;
            else
                comps3DRot=zeros(size(comps3DVoxBDLR23HT_Dnsz_Rsz{validTemplates(j)}));
                for l=1:size(comps3DVoxBDLR23HT_Dnsz_Rsz{validTemplates(j)},3)
                    comps3DRot(:,:,l)=rot90(comps3DVoxBDLR23HT_Dnsz_Rsz{validTemplates(j)}(:,:,l),k-1);
                end
                count=count+1;
    ...
    parfor i=1:length(taggedIndices)
        i;
        candidate=taggedIndices(i);
        %sizesForComp = C{candidate}{4} - C{candidate}{3};
        %xySorted = sort(sizesForComp(1:2), 2);
        %height = sizesForComp(3);
        if 1%isempty(comps3DVoxBDLR23HT_Dnsz_Rsz{candidate}) || (xySorted(1)<10 || xySorted(2)<10 || height<15 || height>50 || xySorted(1)>90 || xySorted(2)>90) || isempty(comps3DVoxBDLR23HT_Dnsz_Rsz{candidate}) || isempty(aaBDLR2{candidate})
            %if ~isempty(comps3DVoxBDLR23HT_Dnsz_Rsz{candidate}) && (xySorted(1)>=10 && xySorted(2)>=10 && height>13 && height<75 && xySorted(1)<=90 && xySorted(2)<=90)&&~isempty(comps3DVoxBDLR23HT_Dnsz_Rsz{candidate}) && ~isempty(aaBDLR2{candidate})
                i;
                newTaggedIndices(i)=1;
                 finalScores{i}=evaluateSimpVoxel2(validTemplates',taggedIndices(i),comps3DVoxBDLR23HT_Dnsz_Rsz,compsHeightViewsAllBDLR23HT_Dnsz,0,templateMat,validTemplates');
                %finalScores{i}=ones([1 length(validTemplates)]).*100000000;
                %end    
        else
            %oneSc=evaluateSimpVoxel2(validTemplates',taggedIndices(i), comps3DVoxBDLR23HT_Dnsz_Rsz,aaBDLR2,C,templateMat,validTemplates');
    ...
    %    height = sizesForComp(3);
    %    scores=zeros(0,4);
    %    if isempty(comps3DVoxBDLR23HT_Dnsz_Rsz{candidate}) || (xySorted(1)<10 || xySorted(2)<10 || height<15 || height>50 || xySorted(1)>90 || xySorted(2)>90) || isempty(comps3DVoxBDLR23HT_Dnsz_Rsz{candidate}) || isempty(aaBDLR2{candidate})
    %        finalScores{i}=ones([1 length(validTemplates)]).*100000000;
    %    else
