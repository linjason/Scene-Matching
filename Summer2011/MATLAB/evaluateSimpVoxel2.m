...
%    load('../../dataStructureForStatistics/bedrooms_2_with_dist_nametags');
    ...
    %scores=zeros(0,4);
    i
    if 0%isempty(comps3DVoxPorDnsized{i}) || (xySorted(1)<10 || xySorted(2)<10 || height<12 || height>90 || xySorted(1)>90 || xySorted(2)>90) || isempty(comps3DVoxPorDnsized{i}) || isempty(aa{i})
        nearest=0;
        scores=100000000;
        disp('uh oh 2');
    else
        ...
        for k=1:4   
            ...
            if isequal(preComputedSearch,search)
        ...
        for j=1:length(search)
            if isempty(comps3DVoxPorDnsized{search(j)})
                ...
                if candidate==search(j)
                    scores(ind,:)=[10000000 100000000 100000000 100000000];
                end
            end
        end
        ...
        %disp(scores)
        scores=min(scores');
        %disp('tpijqijtqi35yjqi53jyq53pjy')
        %disp(scores)
        %info=sortrows([scores;validSearched]');
        %nearest=info(1:end,:);
        %nightstand
